	...
	<div class="list-container">
				...
				<ul class="list-contents">
					...
					<li class="posting-info more-line">
						<p>
							...
							<span class="lines c-gris">白菜价: <span class="c-carbon">点击新建页面</span></span> 
					...
					<li class="posting-action">
						<p class="h-block01">
							<span class="items">
								<span class="c-gris">好价状态 | </span>
								<span class="c-carbon pointer" @click="setStatus(item)">
									{{item.status===0? "隐藏" : item.status===1? "正常" : "失效"}}
								</span>
							</span>
							<span class="items">
								<span class="c-gris">好价归类 | </span>
		...
		>
				<div class="pop-wrapper">
					...
					<status :id="itemData.id" @reload="getCouponsData" v-if="2 === popNum"></status>
					<connection :id="itemData.id" :connectionPro="itemData.products" @reload="getCouponsData" v-if="3 === popNum"></connection>
					<recommend ref="recommend" :id="itemData.id" :haveRecommend="havePulled" :allCommunity="itemData.communities" @reload="getCouponsData" v-if="4 === popNum"></recommend>
					<weight :weight="itemData.weight" :id="itemData.id" @reload="getCouponsData" v-if="7 === popNum"></weight>
