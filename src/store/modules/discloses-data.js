//引入帖子的api
import {communityDisclose, abnormalCommunityDisclose} from "@/axios/api"  //一个首页 一个待审核的列表
...
const actions = {
	...
	//隐藏
	abnormalCommunityDisclose({commit},paras) {
		abnormalCommunityDisclose(paras).then(res => {
			commit('setData', res.data.data)
			commit('setTotal', res.data.meta.total)
			commit('setPage', res.data.meta.current_page)
		})
