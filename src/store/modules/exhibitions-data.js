//引入帖子的api
import {communityQshow, abnormalCommunityQshow} from "@/axios/api"  //一个首页 一个待审核的列表
...
const actions = {
	...
	//隐藏
	getAbnormalQsData({commit},paras) {
		abnormalCommunityQshow(paras).then(res => {
			commit('setData', res.data.data)
			commit('setTotal', res.data.meta.total)
			commit('setPage', res.data.meta.current_page)
		})
